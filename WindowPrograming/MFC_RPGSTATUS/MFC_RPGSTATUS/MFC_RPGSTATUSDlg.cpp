...
// MFC_RPGSTATUSDlg.cpp: 구현 파일
//

#include "pch.h"
#include "framework.h"
#include "MFC_RPGSTATUS.h"
#include "MFC_RPGSTATUSDlg.h"
...
CMFCRPGSTATUSDlg::CMFCRPGSTATUSDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_MFC_RPGSTATUS_DIALOG, pParent)
{
...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT2, m_editHP);
	DDX_Control(pDX, IDC_EDIT3, m_editMP);
	DDX_Control(pDX, IDC_EDIT4, m_editStr);
	DDX_Control(pDX, IDC_EDIT5, m_editInt);
	DDX_Control(pDX, IDC_EDIT6, m_editDef);
...
BEGIN_MESSAGE_MAP(CMFCRPGSTATUSDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN1, &CMFCRPGSTATUSDlg::OnDeltaposSpin1)
END_MESSAGE_MAP()
...
BOOL CMFCRPGSTATUSDlg::OnInitDialog()
{
	...
	strMsg.Format(_T("보너스:%d"), m_nBonusPoint);
...
void CMFCRPGSTATUSDlg::OnDeltaposSpin1(NMHDR* pNMHDR, LRESULT* pResult)
{
